...
function selectRandom() {
  let randomCard = Math.random() * weaponsArray.length;
  let randomPosition = Math.floor(randomCard);
  return weaponsArray[randomPosition];
}

console.log(selectRandom());

function pickMystery() {
  let suspect = suspectsArray[Math.ceil(Math.random() * suspectsArray.length)];
  let weapon = weaponsArray[Math.ceil(Math.random() * weaponsArray.length)];
  let room = roomsArray[Math.ceil(Math.random() * roomsArray.length)];
